from zenoh import Zenoh, Encoding, Change, ChangeKind
import argparse
import cv2
import time
import numpy as np
...
def faces_listener(changes):
    for change in changes:
        ...
        cams[cam][face]["img"] = change.get_value().get_value()
        cams[cam][face]["time"] = time.time()


def names_listener(changes):
    for change in changes:
        chunks = change.get_path().split('/')
        cam = chunks[-3]
        face = int(chunks[-2])
        ...
        cams[cam][face]["name"] = change.get_value().get_value()
...
names = ws.get(args['prefix'] + "/faces/*/*/name", encoding=Encoding.STRING)
names_listener(map(
    lambda name: Change(name.get_path(), ChangeKind.PUT, 0, name.get_value()),
    names))
